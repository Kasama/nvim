...
sy match hs_Type "\<[A-Z]\(\S\&[^,.]\)*\>"
sy match hs_Type "()"
sy match hs_TypeVariable contained "\<[a-z]\(\S&[^,.]\)*\>"
sy match hs_TypeCurry "[=-]>"
sy match hs_TypeCurry contained "[()]"

sy match hs_TypeOp "::"

sy match hs_InlineType "::\s*[A-Z]\w*\(\s+[A-Z]\w*\)*\(\s*-> [A-Z]\w\*\s*\)*" contains=hs_TypeOp,hs_Type,hs_TypeCurry
sy match hs_DeclareFunction "^[a-z_(]\S*\(\s\|\n\)*::.*" contains=hs_FunctionName,hs_OpFunctionName,hs_TypeOp,hs_Type,hs_TypeVariable,hs_TypeCurry
...
sy region  hsPragma           start="{-#" end="#-}" contains=hsPragmaLanguage
sy match   hsPragmaLanguage   contained "LANGUAGE \w*" contains=hsLanguageFeature
sy match   hsLanguageFeature  contained " \w*"
...
if version >= 508 || !exists("did_hs_syntax_inits")
  ...
  HiLink hsPragmaLanguage   Label
  HiLink hsLanguageFeature      Structure
  ...
  HiLink hs_Type            Type
  HiLink hs_TypeVariable    Label
  HiLink hs_TypeCurry       Operator
